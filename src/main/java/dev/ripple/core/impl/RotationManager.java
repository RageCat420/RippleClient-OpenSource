...
import dev.ripple.mod.modules.impl.movement.MovementSync;
...
public class RotationManager implements Wrapper {
    ...
    public void snapBack() {
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), Ripple.ROTATION.rotationYaw, Ripple.ROTATION.rotationPitch, mc.player.isOnGround()));
    }
    ...
    public void snapAt(Vec3d directionVec) {
        ...
        if (AntiCheat.INSTANCE.noSpamRotation.getValue()) {
            if (MathHelper.angleBetween(angle[0], Ripple.ROTATION.lastYaw) < AntiCheat.INSTANCE.fov.getValueFloat() && Math.abs(angle[1] - Ripple.ROTATION.lastPitch) < AntiCheat.INSTANCE.fov.getValueFloat()) {
                return;
            }
        }
    ...
    public void update(UpdateWalkingPlayerEvent event) {
        if (MovementSync.INSTANCE.isOn() && !MovementSync.INSTANCE.updateMode.is(MovementSync.UpdateMode.UpdateMouse) && !BaritoneModule.isActive()) {
            if (event.isPost()) {
                updateNext();
            }
        }
    ...
    public void update(MouseUpdateEvent event) {
        if (mc.player != null && MovementSync.INSTANCE.isOn() && !MovementSync.INSTANCE.updateMode.is(MovementSync.UpdateMode.MovementPacket) && !BaritoneModule.isActive()) {
            updateNext();
        }
    }
    private void updateNext() {
        RotateEvent rotateEvent = new RotateEvent(mc.player.getYaw(), mc.player.getPitch());
        Ripple.EVENT_BUS.post(rotateEvent);
        ...
        MovementSync.fixRotation = nextYaw;
        MovementSync.fixPitch = nextPitch;
    }

    @EventHandler(priority = EventPriority.LOWEST)
    public void onLastRotation(RotateEvent event) {
        LookAtEvent lookAtEvent = new LookAtEvent();
        Ripple.EVENT_BUS.post(lookAtEvent);
