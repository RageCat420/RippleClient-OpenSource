...
import dev.ripple.api.utils.math.ExplosionUtil;
...
import dev.ripple.mod.commands.impl.CoordsCommand;
...
import static dev.ripple.api.utils.world.BlockUtil.*;
...
public class AutoAnchor extends Module {
    ...
    //Assist
    private final BooleanSetting assist = add(new BooleanSetting("Assist", true, () -> page.getValue() == Page.Assist));
    private final BooleanSetting checkMine = add(new BooleanSetting("DetectMining", false, () -> page.getValue() == Page.Assist));
    private final SliderSetting assistRange = add(new SliderSetting("AssistRange", 5.0, 0.0, 6.0, 0.1, () -> page.getValue() == Page.Assist).setSuffix("m"));
    private final SliderSetting assistDamage = add(new SliderSetting("AssistDamage", 6.0, 0.0, 36.0, 0.1, () -> page.getValue() == Page.Assist).setSuffix("h"));
    private final SliderSetting delay = add(new SliderSetting("AssistDelay", 0.1, 0.0, 1, 0.01, () -> page.getValue() == Page.Assist).setSuffix("s"));

    //
    private final BooleanSetting thread = add(new BooleanSetting("Thread", false, () -> page.getValue() == Page.General));
    private final BooleanSetting light = add(new BooleanSetting("LessCPU", true, () -> page.getValue() == Page.General));
    public final SliderSetting range = add(new SliderSetting("Range", 5.0, 0.0, 6.0, 0.1, () -> page.getValue() == Page.General).setSuffix("m"));
    public final SliderSetting targetRange = add(new SliderSetting("TargetRange", 8.0, 0.10, 12, 0.1, () -> page.getValue() == Page.General).setSuffix("m"));
    private final BooleanSetting inventorySwap = add(new BooleanSetting("InventorySwap", true, () -> page.getValue() == Page.General));
    private final BooleanSetting breakCrystal = add(new BooleanSetting("BreakCrystal", true, () -> page.getValue() == Page.General));
    private final BooleanSetting spam = add(new BooleanSetting("Spam", true, () -> page.getValue() == Page.General).setParent());
    private final BooleanSetting mineSpam = add(new BooleanSetting("OnlyMining", true, () -> page.getValue() == Page.General && spam.isOpen()));
    private final BooleanSetting spamPlace = add(new BooleanSetting("Fast", true, () -> page.getValue() == Page.General).setParent());
    private final BooleanSetting inSpam = add(new BooleanSetting("WhenSpamming", true, () -> page.getValue() == Page.General && spamPlace.isOpen()));
    ...
    private final SliderSetting placeDelay = add(new SliderSetting("Delay", 100, 0, 500, 1, () -> page.getValue() == Page.General).setSuffix("ms"));
    private final SliderSetting spamDelay = add(new SliderSetting("SpamDelay", 200, 0, 1000, 1, () -> page.getValue() == Page.General).setSuffix("ms"));
    private final SliderSetting updateDelay = add(new SliderSetting("UpdateDelay", 200, 0, 1000, 1, () -> page.getValue() == Page.General).setSuffix("ms"));

    private final BooleanSetting rotate = add(new BooleanSetting("Rotate", true, () -> page.getValue() == Page.Rotate).setParent());
    private final BooleanSetting yawStep = add(new BooleanSetting("YawStep", true, () -> rotate.isOpen() && page.getValue() == Page.Rotate));
    private final SliderSetting steps = add(new SliderSetting("Steps", 0.05, 0, 1, 0.01, () -> rotate.isOpen() && yawStep.getValue() && page.getValue() == Page.Rotate));
    ...
    private final BooleanSetting noSuicide = add(new BooleanSetting("NoSuicide", true, () -> page.getValue() == Page.Calc));
    private final BooleanSetting terrainIgnore = add(new BooleanSetting("TerrainIgnore", true, () -> page.getValue() == Page.Calc));
    public final SliderSetting minDamage = add(new SliderSetting("Min", 4.0, 0.0, 36.0, 0.1, () -> page.getValue() == Page.Calc).setSuffix("dmg"));
    public final SliderSetting breakMin = add(new SliderSetting("ExplosionMin", 4.0, 0.0, 36.0, 0.1, () -> page.getValue() == Page.Calc).setSuffix("dmg"));
    public final SliderSetting headDamage = add(new SliderSetting("ForceHead", 7.0, 0.0, 36.0, 0.1, () -> page.getValue() == Page.Calc).setSuffix("dmg"));
    private final SliderSetting minPrefer = add(new SliderSetting("Prefer", 7.0, 0.0, 36.0, 0.1, () -> page.getValue() == Page.Calc).setSuffix("dmg"));
    private final SliderSetting maxSelfDamage = add(new SliderSetting("MaxSelf", 8.0, 0.0, 36.0, 0.1, () -> page.getValue() == Page.Calc).setSuffix("dmg"));
    public final SliderSetting predictTicks = add(new SliderSetting("Predict", 2, 0.0, 50, 1, () -> page.getValue() == Page.Calc).setSuffix("ticks"));
    ...
    final BooleanSetting render = add(new BooleanSetting("Render", true, () -> page.getValue() == Page.Render));
    final BooleanSetting shrink = add(new BooleanSetting("Shrink", true, () -> page.getValue() == Page.Render && render.getValue()));
    final ColorSetting box = add(new ColorSetting("Box", new Color(255, 255, 255, 255), () -> page.getValue() == Page.Render && render.getValue()).injectBoolean(true));
    final SliderSetting lineWidth = add(new SliderSetting("LineWidth", 1.5d, 0.01d, 3d, 0.01, () -> page.getValue() == Page.Render && render.getValue()));
    final ColorSetting fill = add(new ColorSetting("Fill", new Color(255, 255, 255, 100), () -> page.getValue() == Page.Render && render.getValue()).injectBoolean(true));
    final SliderSetting sliderSpeed = add(new SliderSetting("SliderSpeed", 0.75, 0d, 1, 0.01, () -> page.getValue() == Page.Render && render.getValue()));
    final SliderSetting startFadeTime = add(new SliderSetting("StartFade", 0.3d, 0d, 2d, 0.01, () -> page.getValue() == Page.Render && render.getValue()).setSuffix("s"));
    final SliderSetting fadeSpeed = add(new SliderSetting("FadeSpeed", 0.2d, 0.01d, 1d, 0.01, () -> page.getValue() == Page.Render && render.getValue()));
    ...
    public AutoAnchor() {
        ...
        setChinese("自动重生锚");
        INSTANCE = this;
        Ripple.EVENT_BUS.subscribe(new AnchorRender());
    }
    ...
    public String getInfo() {
        if (displayTarget != null && currentPos != null && !AutoLog.INSTANCE.getDimension(mc.world).equals("The Nether")) return displayTarget.getName().getString();
        return null;
    }
    ...
    public void onRender3D(MatrixStack matrixStack) {
        if (displayTarget != null && currentPos != null && !AutoLog.INSTANCE.getDimension(mc.world).equals("The Nether")) {
            Aura.doRender(matrixStack, mc.getRenderTickCounter().getTickDelta(true), displayTarget, color.getValue(), mode.getValue());
        }
    }

    @EventHandler
    public void onRotate(LookAtEvent event) {
        if (AutoLog.INSTANCE.getDimension(mc.world).equals("The Nether")) return;
    ...
    public void onThread() {
        if (thread.getValue() && !AutoLog.INSTANCE.getDimension(mc.world).equals("The Nether")) {
            calc();
        }
    }
    ...
    public void onUpdate() {
        ...
        if (currentPos != null) {
            ...
            boolean shouldSpam = this.spam.getValue() && (!mineSpam.getValue() || Ripple.BREAK.isMining(currentPos, false));
            if (shouldSpam) {
    ...
    public enum Page {
        General, Calc, Rotate, Assist, Render,
    }
    ...
    public BlockPos getHelper(BlockPos pos) {
        for (Direction i : Direction.values()) {
            if (checkMine.getValue() && Ripple.BREAK.isMining(pos.offset(i))) continue;
