...
public class PistonCrystal extends Module {
    ...
    private final BooleanSetting noEating = add(new BooleanSetting("NoEating", true).setParent());
    private final BooleanSetting sameHand = add(new BooleanSetting("SameHandPause", true, noEating::isOpen));
    ...
    public void onUpdate() {
        ...
        if (noEating.getValue() && PauseUtil.checkPause(sameHand.getValue()))
            return;
        ...
        if (mc.world.isAir(pos)) {
            if (BlockUtil.canPlace(pos)) {
                if (stage >= pistonStage.getValue() && stage <= pistonMaxStage.getValue()) {
                    ...
                    if (rotate.getValue()) {
                        Ripple.ROTATION.lookAt(neighbour, opposite);
                    }
                }
            } else {
                return;
