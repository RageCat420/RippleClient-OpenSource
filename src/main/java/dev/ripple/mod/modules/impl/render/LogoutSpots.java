...
public class LogoutSpots extends Module {
    ...
    public void onRender3D(MatrixStack matrixStack) {
        for (UUID uuid : logoutCache.keySet()) {
            ...
            if (text.getValue()) {
                Formatting f = Ripple.POP.getFormat(data.getName().getString());
                if (f == null) {
                    if (Ripple.POP.getPop(data.getName().getString()) > 3) {
                        Render3DUtil.drawText3D(data.getName().getString() + " Logged out" + " §6-" + Ripple.POP.getPop(data.getName().getString()), new Vec3d(data.getX(), ((IEntity) data).getDimensions().getBoxAt(data.getPos()).maxY + 0.5, data.getZ()), ColorUtil.injectAlpha(color.getValue(), 255));
                    } else {
                    ...
                    Render3DUtil.drawText3D(data.getName().getString() + " Logged out" + f + " -" + Ripple.POP.getPop(data.getName().getString()), new Vec3d(data.getX(), ((IEntity) data).getDimensions().getBoxAt(data.getPos()).maxY + 0.5, data.getZ()), ColorUtil.injectAlpha(color.getValue(), 255));
                }
            }
        }
    }
