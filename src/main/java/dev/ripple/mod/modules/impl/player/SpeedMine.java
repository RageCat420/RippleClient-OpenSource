...
import dev.ripple.api.events.impl.ClickBlockEvent;
...
import dev.ripple.mod.modules.impl.combat.AutoCev;
...
public class SpeedMine extends Module {
    ...
    private final EnumSetting<SwapType> swap = add(new EnumSetting<>("SwapMode", SwapType.HotBar, () -> page.is(Page.General)));
    private final EnumSetting<Swap> doneSwap = add(new EnumSetting<>("DoneSwap", Swap.Alternative, () -> page.is(Page.General) && swap.is(SwapType.HotBar)));
    private final SliderSetting swapDelay = add(new SliderSetting("SwapDelay", 50, 0, 500, () -> swap.is(SwapType.HotBar) && doneSwap.is(Swap.Alternative) && page.is(Page.General)));
    private final BooleanSetting waitDrop = add(new BooleanSetting("WaitDrop", true, () -> swap.is(SwapType.HotBar) && doneSwap.is(Swap.Alternative) && page.is(Page.General)));
    public final BooleanSetting doubleBreak = add(new BooleanSetting("DoubleBreak", true, () -> page.is(Page.General))).setParent();
    //private final SliderSetting doubleSpeed = add(new SliderSetting("DoubleSwapSpeed", 0.8f, 0f, 2f, () -> page.is(Page.General)));
    ...
    public final BooleanSetting preferHead = add(new BooleanSetting("PreferHead", false, () -> page.is(Page.Check)));
    public final BooleanSetting strictSide = add(new BooleanSetting("StrictSide", false, () -> page.is(Page.Check)));
    ...
    private final BooleanSetting usingPause = add(new BooleanSetting("UsingPause", true, () -> page.is(Page.Check)).setParent());
    private final BooleanSetting onlyMain = add(new BooleanSetting("OnlyMain", true, () -> page.is(Page.Check) && usingPause.isOpen()));
    ...
    public void onUpdate() {
        ...
        if (secondPos != null) {
            ...
            if (secondTimer.passed(time)) {
                ...
                sendSequencedPacket(id -> new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.STOP_DESTROY_BLOCK, secondPos, BlockUtil.getClickSide(secondPos) == null ? Direction.UP : BlockUtil.getClickSide(secondPos), id));
            }
        }
        ...
        if (usingPause.getValue()) {
            if (onlyMain.getValue() && mc.player.isUsingItem() && mc.player.getActiveHand() == Hand.MAIN_HAND) {
                return;
            } else if (!onlyMain.getValue() && mc.player.isUsingItem()) {
                return;
            }
        }
        ...
        if (!swap.is(SwapType.HotBar) && mc.currentScreen != null && !(mc.currentScreen instanceof ChatScreen) && !(mc.currentScreen instanceof InventoryScreen) && !(mc.currentScreen instanceof UIScreen)) {
            return;
        }
        ...
        if (startPacket) {
            ...
            if (done) {
                ...
                if (shouldSwitch) {
                    if (swap.is(SwapType.HotBar)) {
                        if (doneSwap.is(Swap.Silent)) {
                            InventoryUtil.switchToSlot(slot);
                        } else if (doneSwap.is(Swap.Alternative)) {
                            if (!usingPause.getValue() || !PauseUtil.checkPause(onlyMain.getValue())) {
                                InventoryUtil.switchToSlot(slot);
                                oldSlot = old;
                                swapTimer.reset();
                            }
    ...
    private int oldSlot2 = -1;
    private final Timer secondSwapTimer = new Timer();

    @EventHandler
    private void onSecondSwitch(TickEvent event) {
        if (nullCheck()) return;
        if (event.isPost()) return;
        int d = swapDelay.getValueInt();
        if ((waitDrop.getValue() ? (isAir(breakPos) || breakPos == null) : swapTimer.passedMs(d)) && oldSlot != -1 && swap.is(SwapType.HotBar) && doneSwap.is(Swap.Alternative)) {
            InventoryUtil.switchToSlot(oldSlot);
            oldSlot = -1;
        }
        if (!doubleBreak.getValue()) return;
        if ((waitDrop.getValue() ? (isAir(secondPos) || secondPos == null) : secondSwapTimer.passedMs(d)) && oldSlot2 != -1 && swap.is(SwapType.HotBar) && doneSwap.is(Swap.Alternative)) {
            InventoryUtil.switchToSlot(oldSlot2);
            oldSlot2 = -1;
        }
        int slot = getTool(secondPos);
        int old = mc.player.getInventory().selectedSlot;
        if (secondPos == null) return;
        if (usingPause.getValue() && PauseUtil.checkPause(onlyMain.getValue())) return;
        if (MathHelper.sqrt((float) EntityUtil.getEyesPos().squaredDistanceTo(secondPos.toCenterPos())) > range.getValue())
            return;
        boolean done;
        try {
            done = secondTimer.passed(getBreakTime(secondPos, slot, 0.89));
        } catch (Exception e) {
            e.printStackTrace();
            done = false;
        }
        ...
        if (done && !godBlocks.contains(mc.world.getBlockState(secondPos).getBlock()) && secondPos != null) {
            if (shouldSwitch) {
                secondSwapTimer.reset();
                if (swap.is(SwapType.HotBar)) {
                    if (doneSwap.is(Swap.Silent)) {
                        InventoryUtil.switchToSlot(slot);
                    } else if (doneSwap.is(Swap.Alternative)) {
    ...
    public void onAttackBlock(ClickBlockEvent event) {
        ...
        if (BlockUtil.getClickSideStrict(pos) == null && strictSide.getValue()) {
            return;
        }
        ...
        if (swap.is(SwapType.HotBar)) {
            ...
            for (Map.Entry<Integer, ItemStack> entry : InventoryUtil.getInventoryAndHotbarSlots().entrySet()) {
                if (!(entry.getValue().getItem() instanceof AirBlockItem)) {
                    final float digSpeed = EnchantmentHelper.getLevel(mc.world.getRegistryManager().get(Enchantments.EFFICIENCY.getRegistryRef()).getEntry(Enchantments.EFFICIENCY).get(), entry.getValue());
    ...
    public float getDigSpeed(BlockState state, ItemStack itemStack) {
        ...
        if (mc.player.isSubmergedInWater() && EnchantmentHelper.getEquipmentLevel(mc.world.getRegistryManager().get(Enchantments.AQUA_AFFINITY.getRegistryRef()).getEntry(Enchantments.AQUA_AFFINITY).get(), mc.player) <= 0) {
            digSpeed /= 5;
        }
        boolean inWeb = Ripple.PLAYER.isInWeb(mc.player) && mc.world.getBlockState(breakPos).getBlock() == Blocks.COBWEB;
