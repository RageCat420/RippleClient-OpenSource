...
public class AutoPush extends Module {
    ...
    private final BooleanSetting noEating = add(new BooleanSetting("EatingPause", true).setParent());
    private final BooleanSetting sameHand = add(new BooleanSetting("SameHand", true, noEating::isOpen));
    ...
    public static void pistonFacing(Direction i) {
        if (i == Direction.EAST) {
            Ripple.ROTATION.snapAt(-90.0f, 5.0f);
        } else if (i == Direction.WEST) {
            Ripple.ROTATION.snapAt(90.0f, 5.0f);
        } else if (i == Direction.NORTH) {
            Ripple.ROTATION.snapAt(180.0f, 5.0f);
        } else if (i == Direction.SOUTH) {
            Ripple.ROTATION.snapAt(0.0f, 5.0f);
        }
    }
    ...
    public void onUpdate() {
        ...
        if ((noEating.getValue() && mc.player.isUsingItem() && !sameHand.getValue()) || (noEating.getValue() && sameHand.getValue() && mc.player.isUsingItem() && mc.player.getActiveHand() == Hand.MAIN_HAND))
            return;
            ...
            if (BlockUtil.clientCanPlace(piston)) {
                ...
                if (BlockUtil.getPlaceSide(piston, placeRange.getValue()) != null) {
                    ...
                    if (canPower) {
                        ...
                        if (side != null) {
                            if (rotate.getValue()) Ripple.ROTATION.lookAt(piston.offset(side), side.getOpposite());
                            ...
                            if (rotate.getValue() && yawDeceive.getValue()) Ripple.ROTATION.lookAt(piston.offset(side), side.getOpposite());
...
/*                        if (mine.getValue()) {
    ...
    private boolean isTrueFacing(BlockPos pos, Direction facing) {
        ...
        float[] rotation = Ripple.ROTATION.getRotation(directionVec);
    ...
    private Boolean canPush(PlayerEntity player) {
        ...
        if (!allowWeb.getValue() && Ripple.PLAYER.isInWeb(player)) return false;
        float[] offset = new float[]{-0.25f, 0f, 0.25f};
        ...
        return progress > surroundCheck.getValue() - 1 || Ripple.HOLE.isHard(new BlockPosX(player.getX(), player.getY() + 0.5, player.getZ()));
    }
