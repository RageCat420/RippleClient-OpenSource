...
import dev.ripple.api.events.impl.PlaySoundEvent;
...
public class NoSoundLag extends Module {
	...
	static final ArrayList<RegistryEntry<SoundEvent>> armor = new ArrayList<>();
	@EventHandler
	public void onPlaySound(PlaySoundEvent event){
		if (equip.getValue()) {
			for (RegistryEntry<SoundEvent> se : armor) {
				if (event.sound.getId() == se.value().getId()) {
					event.cancel();
					return;
				}
			}
