...
public class BedAura extends Module {
	...
	private final BooleanSetting noUsing =
			add(new BooleanSetting("EatingPause", true, () -> page.getValue() == Page.General).setParent());
	private final BooleanSetting sameHand = add(new BooleanSetting("SameHand", true, () -> page.getValue() == Page.General && noUsing.isOpen()));
	...
	public void onEnable() {
		lastYaw = Ripple.ROTATION.lastYaw;
		lastPitch = Ripple.ROTATION.lastPitch;
	}
	...
	public void onRotate(RotateEvent event) {
		if (!rotate.getValue() && yawDeceive.getValue()) {
			...
			float[] newAngle = injectStep(Ripple.ROTATION.getRotation(directionVec), steps.getValueFloat());
			lastYaw = newAngle[0];
	...
	private void update() {
		...
		if ((noUsing.getValue() && mc.player.isUsingItem() && !sameHand.getValue()) || (noUsing.getValue() && mc.player.isUsingItem() && sameHand.getValue() && mc.player.getActiveHand() == Hand.MAIN_HAND)) {
			placePos = null;
			return;
		}
	...
	private void doPlace(BlockPos pos) {
		...
		for (Direction i : Direction.values()) {
			...
			if ((legacy.getValue() && BlockUtil.canReplace(pos.offset(i)) || BlockUtil.clientCanPlace(pos.offset(i), false)) && BlockUtil.canClick(pos.offset(i).down()) && (!checkMine.getValue() || !Ripple.BREAK.isMining(pos.offset(i)))) {
				facing = i;
				break;
			}
		}
	...
	private boolean canPlaceBed(BlockPos pos) {
		if (BlockUtil.canReplace(pos) && (!checkMine.getValue() || !Ripple.BREAK.isMining(pos)) && (!legacy.getValue() || BlockUtil.canClick(pos.down()))) {
			for (Direction i : Direction.values()) {
				if (i == Direction.UP || i == Direction.DOWN) continue;
	...
	private boolean isTrueFacing(BlockPos pos, Direction facing) {
		...
		return Direction.fromRotation(Ripple.ROTATION.getRotation(hitVec)[0]) == facing;
	}

	private boolean isTrueFacingNow(Direction facing) {
		return Direction.fromRotation(Ripple.ROTATION.lastYaw) == facing;
	}
	public enum Page {
		General,
		Rotate,
	...
	public boolean faceVector(Vec3d directionVec) {
		if (!yawStep.getValue()) {
			...
			if (Ripple.ROTATION.inFov(directionVec, fov.getValueFloat())) return true;
		}
