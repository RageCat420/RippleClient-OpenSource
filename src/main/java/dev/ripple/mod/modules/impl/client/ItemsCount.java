...
public class ItemsCount extends Module {
    ...
    public final BooleanSetting ladder = add(new BooleanSetting("Ladder", true));
    public final BooleanSetting scaffolding = add(new BooleanSetting("Scaffolding", true));
    public final BooleanSetting itemFrame = add(new BooleanSetting("ItemFrame", true));
    public final BooleanSetting rocket = add(new BooleanSetting("Rocket", true));
    ...
    ItemStack ladderStack = new ItemStack(Items.LADDER);
    ItemStack scaffoldingStack = new ItemStack(Items.SCAFFOLDING);
    ItemStack itemFrameStack = new ItemStack(Items.ITEM_FRAME);
    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        ...
        if (ladder.getValue()) {
            ladderStack.setCount(getItemCount(Items.LADDER));
            drawItem(ladderStack);
        }
        if (scaffolding.getValue()) {
            scaffoldingStack.setCount(getItemCount(Items.SCAFFOLDING));
            drawItem(scaffoldingStack);
        }
        if (itemFrame.getValue()) {
            int frameCount = InventoryUtil.getItemCount(Items.ITEM_FRAME) + InventoryUtil.getItemCount(Items.GLOW_ITEM_FRAME);
            if (frameCount > 0 || !hideEmpty.getValue()) {
                x += offset.getValueInt();
                itemFrameStack.setCount(Math.max(1, frameCount));
                drawItem(itemFrameStack);
            }
        }
        if (rocket.getValue()) {
            rocketStack.setCount(getItemCount(Items.FIREWORK_ROCKET));
            drawItem(rocketStack);
        }
    }
