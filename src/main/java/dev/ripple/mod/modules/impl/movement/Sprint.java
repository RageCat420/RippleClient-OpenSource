			...
			switch (mode.getValue()) {
				case Grim -> {
					if (MovementSync.INSTANCE.isOn()) {
						...
						return HoleSnap.INSTANCE.isOn() || mc.options.forwardKey.isPressed() && MathHelper.angleBetween(mc.player.getYaw(), Ripple.ROTATION.rotationYaw) < 40;
					}
				}
				case Rotation -> {
					if (MovementSync.INSTANCE.isOn()) {
						...
						return HoleSnap.INSTANCE.isOn() || MathHelper.angleBetween(getSprintYaw(mc.player.getYaw()), Ripple.ROTATION.rotationYaw) < 40;
					}
				}
				case Rage -> {
					return true;
