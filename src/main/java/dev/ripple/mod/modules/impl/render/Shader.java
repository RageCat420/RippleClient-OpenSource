...
public class Shader extends Module {
    ...
    public final ColorSetting fill = add(new ColorSetting("Color", new Color(0, 200, 255, 100), () -> page.getValue() == Page.Shader));

    public final SliderSetting maxSample = add(new SliderSetting("MaxSample", 10.0f, 0f, 20.0f, () -> page.getValue() == Page.Shader));
    public final SliderSetting divider = add(new SliderSetting("Divider", 150.0f, 0f, 300.0f, () -> page.getValue() == Page.Shader));

    public final SliderSetting radius = add(new SliderSetting("Radius", 2.0f, 0f, 6.0f, () -> page.getValue() == Page.Shader));
    public final SliderSetting smoothness = add(new SliderSetting("Smoothness", 1.0f, 0f, 1.0f, 0.01, () -> page.getValue() == Page.Shader));
    public final SliderSetting alpha = add(new SliderSetting("GlowAlpha", 75, 0, 255, () -> page.getValue() == Page.Shader));
    ...
    public boolean shouldRender(Entity entity) {
        ...
        if (MathHelper.sqrt((float) mc.player.squaredDistanceTo(entity.getPos())) > maxRange.getValue()) return false;

        if (entity instanceof PlayerEntity) {
            if (entity == mc.player) return self.getValue();
            if (Ripple.FRIEND.isFriend((PlayerEntity) entity)) return friends.getValue();
    ...
    public void onRender3D(MatrixStack matrixStack) {
        if (hands.getValue()) {
            Ripple.SHADER.renderShader(() -> mc.gameRenderer.renderHand(mc.gameRenderer.getCamera(), mc.getRenderTickCounter().getTickDelta(true), matrixStack.peek().getPositionMatrix()), mode.getValue());
        }
    }

    @Override
    public void onToggle() {
        Ripple.SHADER.reloadShaders();
    }
