...
import static dev.ripple.api.utils.entity.MovementUtil.*;
...
public class ElytraFly extends Module {
    ...
    private final BooleanSetting usingPause = add(new BooleanSetting("UsingPause", false, () -> !mode.is(Mode.Bounce) && firework.getValue()).setParent());
    private final BooleanSetting same = add(new BooleanSetting("SameHand", false, () -> usingPause.isOpen() && usingPause.visibility.getAsBoolean()));
    ...
    public void onUpdate() {
        ...
        if (mode.is(Mode.Bounce)) {
            ...
            if (firework.getValue() && fireworkTimer.passed(delay.getValueInt()) && MovementUtil.isMoving() && (!usingPause.getValue() || !PauseUtil.checkPause(same.getValue())) && mc.player.isFallFlying()) {
