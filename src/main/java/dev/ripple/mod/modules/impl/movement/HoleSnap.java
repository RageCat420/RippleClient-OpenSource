...
public class HoleSnap extends Module {
   ...
   private final SliderSetting priority = add(new SliderSetting("Priority", 10,0 ,100, grim::getValue));
    ...
    public void onEnable() {
        ...
        resetMove = false;
        holePos = Ripple.HOLE.getHole((float) range.getValue(), true, any.getValue(), up.getValue());
    }
    ...
    public void onKeyInput(KeyboardInputEvent e) {
        if (!grim.getValue()) {
            return;
        } else if (MovementSync.INSTANCE.isOff() || !MovementSync.INSTANCE.grim.getValue()) {
    ...
    public void onUpdate() {
        ...
        if (Ripple.HOLE.isDoubleHole(holePos)) {
            Direction facing = Ripple.HOLE.is3Block(holePos);
            if (facing != null) {
