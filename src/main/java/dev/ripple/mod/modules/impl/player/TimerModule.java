...
public class TimerModule extends Module {
	...
	public void onRender2D(DrawContext drawContext, float tickDelta) {
		...
		if (MovementUtil.isMoving() && !Ripple.PLAYER.insideBlock) {
			...
			if (timer2.passed(lastMs)) {
				...
				if (smooth.getValue()) {
					double timer = Ripple.TIMER.getDefault() + (1 - end.ease(ease.getValue())) * (shiftTimer.getValueFloat() - 1) * (lastMs / accumulate.getValue());
					Ripple.TIMER.set((float) Math.max(Ripple.TIMER.getDefault(), timer));
				} else {
					Ripple.TIMER.set(shiftTimer.getValueFloat());
				}
			}
		} else {
			if (moving) {
