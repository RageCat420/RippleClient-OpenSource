...
import dev.ripple.mod.modules.impl.combat.SelfTrap;
import dev.ripple.mod.modules.impl.combat.FeetPlace;
...
public class Step extends Module {
    ...
    public void onUpdate() {
		if (pathingPause.getValue() && BaritoneModule.isActive() || sneakingPause.getValue() && mc.player.isSneaking() || inBlockPause.getValue() && Ripple.PLAYER.insideBlock || mc.player.isInLava() || mc.player.isTouchingWater() || inWebPause.getValue() && Ripple.PLAYER.isInWeb(mc.player) || !mc.player.isOnGround() || onlyMoving.getValue() && !MovementUtil.isMoving() || surroundPause.getValue() && (FeetPlace.INSTANCE.isOn() || SelfTrap.INSTANCE.isOn())) {
			mc.player.getAttributeInstance(EntityAttributes.GENERIC_STEP_HEIGHT).setBaseValue(0.6f);
			return;
		}
		...
		if (mode.getValue().equals(Mode.OldNCP) || strict) {
			...
			if (offsets != null && offsets.length > 1) {
				if (useTimer.getValue()) {
					Ripple.TIMER.set((float) getTimer(stepHeight));
					timer = true;
					packets = 2;
				}
