...
import static dev.ripple.api.utils.world.BlockUtil.canReplace;
...
public class Burrow extends Module {
    ...
    public void onUpdate() {
        if (Ripple.PLAYER.isInWeb(mc.player)) {
            webTimer.reset();
            return;
        }
    ...
    private void gotoPos(BlockPos offPos) {
        ...
        if (rotate.getValue() == RotateMode.None) {
            ...
            mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(offPos.getX() + 0.5, mc.player.getY() + 0.1, offPos.getZ() + 0.5, Ripple.ROTATION.rotationYaw, 90, false));
        }
    }
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        if (detectMine.getValue() && Ripple.BREAK.isMining(pos)) {
            return false;
        }
