...
public class HoleFiller extends Module {
    ...
    public void onUpdate() {
        ...
        CombatUtil.getEnemies(enemyRange.getValue()).stream()
                ...
                .filter(pos -> pos.toCenterPos().distanceTo(mc.player.getPos()) > selfRange.getValue() && (Ripple.HOLE.isHole(pos, true, true, false) || Ripple.HOLE.isDoubleHole(pos)))
                .distinct()
    ...
    private void tryPlaceBlock(BlockPos pos) {
        if (pos == null) return;
        if (detectMining.getValue() && Ripple.BREAK.isMining(pos)) return;
