...
public class ModuleList extends Module {
    ...
    private final BooleanSetting ingoreF = add(new BooleanSetting("IgnoreFastL", true, onlyBind::getValue));
    ...
    private final ColorSetting color = add(new ColorSetting("Color", new Color(0, 120, 255, 255), () -> colorMode.getValue() != ColorMode.Rainbow));
    private final ColorSetting endColor = add(new ColorSetting("EndColor", new Color(0, 60, 200, 255), () -> colorMode.getValue() == ColorMode.Pulse));
    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        ...
        for (Modules modules : modulesList) {
            if (modules.module.isOn() && modules.module.drawnSetting.getValue() && !(onlyBind.getValue() && modules.module.getBind().getKey() == -1)) {
                ...
                if (modules.module.getName().contains("FastLatency") && ingoreF.getValue()
                        && modules.module.isOn() && modules.module.drawnSetting.getValue()) {
                    modules.enable();
                } else {
                    modules.disable();
                }
