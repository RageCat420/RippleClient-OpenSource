...
import dev.ripple.mod.modules.impl.client.BaritoneModule;
...
public class Speed extends Module {
    ...
    public enum Mode {
        Custom, GrimCollide, StrafeStrict,
    }
    ...
    private final BooleanSetting inWater = add(new BooleanSetting("InWater", false, () -> !mode.is(Mode.GrimCollide)));
    private final BooleanSetting inBlock = add(new BooleanSetting("InBlock", false, () -> !mode.is(Mode.GrimCollide)));
    private final BooleanSetting airStop = add(new BooleanSetting("AirStop", false, () -> !mode.is(Mode.GrimCollide)));
    private final SliderSetting lagTime = add(new SliderSetting("LagTime", 500, 0, 1000, 1, () -> !mode.is(Mode.GrimCollide)));

    private final BooleanSetting jump = add(new BooleanSetting("Jump", true, () -> mode.is(Mode.Custom)));
    private final SliderSetting strafeSpeed = add(new SliderSetting("Speed", 0.2873, 0, 1.0, 0.0001, () -> mode.is(Mode.Custom)));
    private final BooleanSetting explosions = add(new BooleanSetting("ExplosionsBoost", false, () -> mode.is(Mode.Custom)));
    private final BooleanSetting velocity = add(new BooleanSetting("VelocityBoost", true, () -> mode.is(Mode.Custom)));
    private final SliderSetting multiplier = add(new SliderSetting("H-Factor", 1.0f, 0.0f, 5.0f, 0.01, () -> mode.is(Mode.Custom)));
    private final SliderSetting vertical = add(new SliderSetting("V-Factor", 1.0f, 0.0f, 5.0f, 0.01, () -> mode.is(Mode.Custom)));
    private final SliderSetting coolDown = add(new SliderSetting("CoolDown", 1000, 0, 5000, 1, () -> mode.is(Mode.Custom)));
    private final BooleanSetting slow = add(new BooleanSetting("Slowness", false, () -> mode.is(Mode.Custom)));
    ...
    public void invoke(PacketEvent.Receive event) {
        ...
        if (mode.is(Mode.Custom)) {
            if (event.getPacket() instanceof EntityVelocityUpdateS2CPacket packet) {
                if (mc.player != null && packet.getEntityId() == mc.player.getId() && this.velocity.getValue()) {
                    ...
                    if (this.lastExp > 0) {
                        ...
                        mc.executeTask(() -> {
                            ...
                            if (MovementUtil.getMotionY() > 0 && this.vertical.getValue() != 0) {
                ...
                if (this.explosions.getValue() && MovementUtil.isMoving()) {
                    if (mc.player.squaredDistanceTo(packet.getX(), packet.getY(), packet.getZ()) < 200) {
                        double speed = Math.sqrt(Math.abs(packet.getPlayerVelocityX() * packet.getPlayerVelocityX()) + Math.abs(packet.getPlayerVelocityZ() * packet.getPlayerVelocityZ()));
                        this.lastExp = this.expTimer.passedMs(this.coolDown.getValueInt()) ? speed : (speed - this.lastExp);
    ...
    public void invoke(MoveEvent event) {
        if (Glide.INSTANCE != null && Glide.INSTANCE.isOn() && mc.player.hasStatusEffect(StatusEffects.SLOW_FALLING))
            return;
        if (!MovementUtil.isMoving() && airStop.getValue() && !mode.is(Mode.GrimCollide)) {
            MovementUtil.setMotionX(0);
            MovementUtil.setMotionZ(0);
        }
        if (!this.inWater.getValue() && (mc.player.isSubmergedInWater() || mc.player.isTouchingWater() || mc.player.isInLava()) || mc.player.isRiding() || mc.player.isHoldingOntoLadder() || !inBlock.getValue() && Ripple.PLAYER.insideBlock || mc.player.getAbilities().flying || mc.player.isFallFlying() || !MovementUtil.isMoving()) {
            resetStrafe();
            this.stop = true;
            return;
        }
        ...
        if (mode.getValue() == Mode.StrafeStrict) {
            ...
            if (strafe == 1) {
                ...
                if ((!mc.world.isSpaceEmpty(mc.player, mc.player.getBoundingBox().offset(0, mc.player.getVelocity().getY(), 0)) || mc.player.verticalCollision) && strafe > 0) {
                    strafe = 1;
                }
