...
public class HoleESP extends Module {
	...
	private final SliderSetting updateDelay = add(new SliderSetting("UpdateDelay", 50, 0, 1000));
	...
	Type isHole(BlockPos pos) {
		if (mc.world.isAir(pos) && (!airYCheck.getValue() || pos.getY() == mc.player.getBlockY() - 1) && Ripple.HOLE.isHard(pos.up())) return Type.Air;
		int blockProgress = 0;
		boolean bedRock = true;
		for (Direction i : Direction.values()) {
			if (i == Direction.UP || i == Direction.DOWN) continue;
			if (Ripple.HOLE.isHard(pos.offset(i))) {
		...
		if (Ripple.HOLE.isDoubleHole(pos)) return Type.Normal;
		return Type.None;
	}
