package dev.ripple.api.utils.math;

import dev.ripple.Ripple;
import dev.ripple.core.impl.PlayerManager;
...
public class ExplosionUtil implements Wrapper {
    ...
    public static float getBlastReduction(LivingEntity entity, float damageI) {
        ...
        if (entity instanceof PlayerEntity player) {
            PlayerManager.EntityAttribute a = Ripple.PLAYER.map.get(player);
            if (a == null) return 0;
...
/*            if (player.hasStatusEffect(StatusEffects.RESISTANCE)) {
    ...
    public static int getProtectionAmount(Iterable<ItemStack> armorItems) {
        ...
        for (ItemStack itemStack : new ArrayList<>((DefaultedList<ItemStack>) armorItems)) {
            int level = EnchantmentHelper.getLevel(mc.world.getRegistryManager().get(Enchantments.PROTECTION.getRegistryRef()).getEntry(Enchantments.PROTECTION).get(), itemStack);
            if (level == 0) {
                value += EnchantmentHelper.getLevel(mc.world.getRegistryManager().get(Enchantments.BLAST_PROTECTION.getRegistryRef()).getEntry(Enchantments.BLAST_PROTECTION).get(), itemStack) * 2;
            } else {
                value += level;
            }
        }
