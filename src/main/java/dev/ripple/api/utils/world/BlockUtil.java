package dev.ripple.api.utils.world;
...
import dev.ripple.mod.modules.impl.combat.AutoWeb;
...
public class BlockUtil implements Wrapper {
    ...
    public static boolean hasEntity(BlockPos pos, boolean ignoreCrystal) {
        for (Entity entity : getEntities(new Box(pos))) {
            if (!entity.isAlive() || entity instanceof ItemFrameEntity || entity instanceof GlowItemFrameEntity || entity instanceof ItemEntity || entity instanceof ExperienceOrbEntity || entity instanceof ExperienceBottleEntity || entity instanceof ArrowEntity || ignoreCrystal && entity instanceof EndCrystalEntity || entity instanceof ArmorStandEntity && AntiCheat.INSTANCE.obsMode.getValue())
                continue;
            return true;
        }
        return false;
    ...
    public static void placeBlockGround(BlockPos pos, boolean rotate, boolean packet) {
        if (!canClick(pos.offset(Direction.DOWN))) return;
        placedPos.add(pos);
        clickBlock(pos.offset(Direction.DOWN), Direction.UP, rotate, Hand.MAIN_HAND, packet);
    }
