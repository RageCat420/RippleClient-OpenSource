...
public class JelloUtil implements Wrapper {
    ...
    public static void drawJello(MatrixStack matrix, Entity target, Color color) {
        ...
        double x = target.prevX + (target.getX() - target.prevX) * mc.getRenderTickCounter().getTickDelta(true) - mc.getEntityRenderDispatcher().camera.getPos().getX();
        double y = target.prevY + (target.getY() - target.prevY) * mc.getRenderTickCounter().getTickDelta(true) - mc.getEntityRenderDispatcher().camera.getPos().getY() + prevSinAnim * target.getHeight();
        double z = target.prevZ + (target.getZ() - target.prevZ) * mc.getRenderTickCounter().getTickDelta(true) - mc.getEntityRenderDispatcher().camera.getPos().getZ();
        double nextY = target.prevY + (target.getY() - target.prevY) * mc.getRenderTickCounter().getTickDelta(true) - mc.getEntityRenderDispatcher().camera.getPos().getY() + sinAnim * target.getHeight();
