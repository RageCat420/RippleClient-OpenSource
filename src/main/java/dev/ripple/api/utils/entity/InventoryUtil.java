...
public class InventoryUtil implements Wrapper {
    ...
    public static int getPotionCount(RegistryEntry<StatusEffect> targetEffect) {
        int count = 0;
        for (int i = 0; i < 45; ++i) {
    ...
    public static boolean silentSwapEquipChestplate() {
        if (!mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem().equals(Items.NETHERITE_CHESTPLATE) &&
                !mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem().equals(Items.DIAMOND_CHESTPLATE)) {

            int hotbarChestplateSlot = findItem(Items.NETHERITE_CHESTPLATE);
            if (hotbarChestplateSlot == -1) {
                hotbarChestplateSlot = findItem(Items.DIAMOND_CHESTPLATE);
            }

            if (hotbarChestplateSlot != -1) {
                mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, 6, hotbarChestplateSlot, SlotActionType.SWAP, mc.player);
                EntityUtil.syncInventory();
                return true;
            } else {
                int inventorySlot = findItemInventorySlot(Items.NETHERITE_CHESTPLATE);
                if (inventorySlot == -1) {
                    inventorySlot = findItemInventorySlot(Items.DIAMOND_CHESTPLATE);
                }
    ...
    public static boolean silentSwapEquipElytra() {
        if (mc.player.getEquippedStack(EquipmentSlot.CHEST).getItem().equals(Items.ELYTRA)) {
            ...
            int inventorySlot = findItem(Items.ELYTRA);
            if (inventorySlot != -1) {
                mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, 6, inventorySlot, SlotActionType.SWAP, mc.player);
                EntityUtil.syncInventory();
                return true;
            } else {
                inventorySlot = findItemInventorySlot(Items.ELYTRA);
                if (inventorySlot == -1) {
                    return false;
                } else {
                    mc.interactionManager.clickSlot(mc.player.currentScreenHandler.syncId, inventorySlot, 0, SlotActionType.PICKUP, mc.player);
